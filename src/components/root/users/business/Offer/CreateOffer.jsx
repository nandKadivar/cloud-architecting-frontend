  ...
  const submitHandler = (event) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      const uploaders = filesToUpload.map(async (file) => {
        ...
        const key = "img-"+Math.floor(Math.random() * 99999999)+".jpg"
        const params = {
          Bucket: S3_BUCKET,
          Key: key,
          Body: file,
        ...
        return await s3.putObject(params).promise().then((err, data) => {
            fileURL = "https://term-project-application.s3.amazonaws.com/"+key;
            const public_id = key;
            console.log("Inside Data Inside Data Inside Data")
